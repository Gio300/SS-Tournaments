...
export function MatchDetailClient() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      ...
      {liveStreamId && (
        <div className="mb-8">
          <h2 className="font-semibold text-text-primary mb-4">Live Stream</h2>
      ...
      {(youtubeClips.length > 0 || uploadClips.length > 0) && (
        <div className="space-y-4 mb-8">
          <h2 className="font-semibold text-text-primary">Clips</h2>
          <div className="grid gap-4">
            {youtubeClips.map((clip) => (
              <YouTubeEmbed key={clip.id} clip={clip} />
            ))}
            {uploadClips.map((clip) => (
              <div key={clip.id} className="aspect-video rounded-lg overflow-hidden border border-border">
      ...
      {reels.length > 0 && (
        <div>
          <h2 className="font-semibold text-text-primary mb-4">Reels</h2>
          <div className="space-y-6">
            {reels.map((reel) => (
              <div key={reel.id} className="rounded-xl border border-border bg-panel overflow-hidden">
                <h3 className="p-4 font-medium text-text-primary">{reel.title}</h3>
      ...
      <Link href="/matches/" className="inline-block mt-8 text-accent hover:underline">← Back to Matches</Link>
    </div>
  )
}
