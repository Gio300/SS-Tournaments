...
-- Includes: profiles, clips, reels, matches, servers, posts, tournaments,
-- tournament_admins, stat_check_submissions, tournament_results, trophies,
-- post_reactions, post_comments, shareable posts/reposts, storage buckets
-- ============================================
...
-- Post reactions and comments (Facebook-style)
create table if not exists public.post_reactions (id uuid default uuid_generate_v4() primary key, post_id uuid references public.posts(id) on delete cascade not null, user_id uuid references public.profiles(id) on delete cascade not null, emoji text not null check (emoji in ('like', 'love', 'haha', 'wow', 'sad', 'angry')), created_at timestamptz default now(), unique(post_id, user_id));
create table if not exists public.post_comments (id uuid default uuid_generate_v4() primary key, post_id uuid references public.posts(id) on delete cascade not null, user_id uuid references public.profiles(id) on delete cascade not null, body text not null, created_at timestamptz default now());
...
-- Tournament admins, stat check under tournaments, tournament results
